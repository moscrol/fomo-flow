...
async function main() {
  ...
  const scenarios = [
    ...
    {
      name: "malformed_edit_repair",
      msg: "Please make a malformed edit missing args so validation can repair it",
      expectTool: "Edit",
      expectArgs: ["file_path", "old_string", "new_string"],
      expectStop: W.STOP_TOOL_CALLS,
    },
    {
      name: "write_to_file",
  ...
  for (const sc of scenarios) {
    ...
    try {
      ...
      if (sc.expectThinkingHidden)
        ok(`${sc.name}: reasoning hidden`, parsed.thinking.length === 0, "thinking was exposed");
      if (sc.expectTool)
        ok(
          `${sc.name}: 工具`,
      ...
      if (sc.expectArgs) {
        const selected = parsed.toolCalls.find(
          (tool) => tool.name === sc.expectTool,
        );
        let args = {};
        try {
          args = JSON.parse((selected && selected.argumentsJson) || "{}");
        } catch {}
        ok(
          `${sc.name}: 修复后参数完整`,
          sc.expectArgs.every((name) =>
  ...
  console.log("\n── 5.5c 主流式空闲保活 idle-keepalive ──");
  {
    ...
    try {
      ...
      ok(
        "保活启用: 元数据帧不暴露 thinking",
        on.parsed.thinking.length === 0 && off.parsed.thinking.length === 0,
