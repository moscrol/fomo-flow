...
function apply(params) {
  ...
  const systemBudget = Math.floor(maxCtx * (budget.systemBudgetRatio || 0));
  let system = budget.truncateDescriptions && rawSystem
    ? _compactPromptText(rawSystem)
    : rawSystem || "";
  let systemTokens = countTokens(system, encoder);
  if (systemBudget > 0 && systemTokens > systemBudget) {
    system = _truncateToTokenBudget(system, systemBudget, encoder);
    systemTokens = countTokens(system, encoder);
  }
  ...
  // ── 3. 历史预算: ratio 桶与「去掉 system/tools/reserve 的剩余」取小
  //    不再从 history 桶里再减 system/tools/reserve (那会双重扣减、过度裁剪)
  const leftover = Math.max(0, maxCtx - systemTokens - toolTokens - reserveTokens);
  const availableForHistory = Math.min(historyBudget, leftover);
...
function applyHistoryBudget(messages, budget, encoder) {
  ...
  for (let i = 0; i < chatMsgs.length; i++) {
    ...
    if (
      ...
      while (
        i + 1 < chatMsgs.length &&
        chatMsgs[i + 1].role === "tool" &&
        requiredIds.has(chatMsgs[i + 1].tool_call_id)
      ) {
        unit.push(chatMsgs[++i]);
      }
      units.push(unit);
  ...
  let totalTokens = 0;
  const keptUnits = [];
  let trimmed = orphanCount;
  for (let i = units.length - 1; i >= 0; i--) {
    const unit = units[i];
    const unitTokens = unit.reduce(
      (sum, message) => sum + countMessageTokens(message, encoder),
      0,
    );
    if (totalTokens + unitTokens > budget) {
      trimmed += units
        .slice(0, i + 1)
        .reduce((sum, earlierUnit) => sum + earlierUnit.length, 0);
      break;
    }
...
function _truncateToTokenBudget(text, maxTokens, encoder) {
  if (!text || typeof text !== "string" || maxTokens <= 0) return text || "";
  if (countTokens(text, encoder) <= maxTokens) return text;
  let lo = 0;
  let hi = text.length;
  let best = "";
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const cut = text.slice(0, mid);
    if (countTokens(cut, encoder) <= maxTokens) {
      best = cut;
      lo = mid + 1;
    } else {
      hi = mid - 1;
