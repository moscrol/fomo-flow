...
const responsesState = require("./responses_state");
...
function _completionPathProtocol(completionPath) {
  const value = String(completionPath || "").toLowerCase();
  if (/\/responses(?:\/|$|\?)/.test(value)) return PROTOCOL.OPENAI_RESPONSES;
  if (/\/messages(?:\/|$|\?)/.test(value)) return PROTOCOL.ANTHROPIC;
  if (/generatecontent/.test(value)) return PROTOCOL.GEMINI;
  if (/\/chat\/completions(?:\/|$|\?)/.test(value)) return PROTOCOL.OPENAI_CHAT;
  return null;
}

function _completionPathFor(provCfg, protocol, fallback) {
  const configured = String((provCfg && provCfg.completionPath) || "").trim();
  if (!configured) return fallback;
  const configuredProtocol = _completionPathProtocol(configured);
  return !configuredProtocol || configuredProtocol === protocol
    ? configured
    : fallback;
}
...
function detectProtocol(provCfg, model) {
  ...
  if (provCfg.type === "gemini") return PROTOCOL.GEMINI;
  ...
  if (cp.includes(":generatecontent") || cp.includes(":streamgeneratecontent"))
    return PROTOCOL.GEMINI;

  // 5. 根据 model 名称推断
  const m = (model || "").toLowerCase();
  if (m.startsWith("gemini")) return PROTOCOL.GEMINI;
    ...
    const _usage = obj.usage
      ? {
          ...
          cached:
            ...
            0,
          cacheWrite: obj.usage.prompt_tokens_details?.cache_write_tokens || 0,
        }
      : null;
      ...
      usage: obj.usage
        ? {
            ...
            cached:
              ...
              0,
            cacheWrite:
              obj.usage.prompt_tokens_details?.cache_write_tokens || 0,
          }
        : null,
    };
  },
...
const AnthropicAdapter = {
  ...
  buildRequest(opts) {
    ...
    if (thinkingEnabled) {
      if (_isAdaptiveAnthropicModel(model)) {
        body.thinking = { type: "adaptive" };
        body.output_config = {
          effort: reasoningEffort || _anthropicEffortFromBudget(thinkingBudget),
        };
      } else {
        body.thinking = {
          type: "enabled",
  ...
  buildRequestOpts(provCfg, body, targetUrl) {
    ...
    // Beta 头: 提示缓存恒启用 · thinking/1h TTL 按需叠加
    //   1h TTL 需要与 cache_control.ttl 配套的 extended-cache-ttl beta 标记。
    headers["anthropic-beta"] = _composeAnthropicBetaHeader(provCfg, body);
   ...
   */
  getCompletionPath(provCfg) {
    return _completionPathFor(provCfg, PROTOCOL.ANTHROPIC, "/v1/messages");
  },
};

function _isAdaptiveAnthropicModel(model) {
  ...
  if (/claude-(?:3|opus-4-[015]|sonnet-4-[05]|haiku-4-5)/.test(name))
    return false;
  if (
    /claude-(?:opus|sonnet)-4-(?:6|7|8|9)|claude-(?:opus|sonnet)-5/.test(name)
  )
    return true;
  return /^claude-/.test(name);
}

function _anthropicEffortFromBudget(thinkingBudget) {
  const value = Number(thinkingBudget) || 0;
...
const OpenAIResponsesAdapter = {
  ...
  buildRequest(opts) {
    ...
    const convertedInput = _convertMessagesToResponsesInput(messages, system);
    const input =
      includeEncryptedReasoning === false
        ? convertedInput
        : responsesState.inject(
            continuationKey || promptCacheKey,
            convertedInput,
          );
    ...
    const body = {
      ...
      input,
      stream,
      // Codex uses stateless Responses calls so encrypted reasoning is not
      // retained by the provider between turns.
      store: responsesStore === true,
    };
    ...
    if (thinkingEnabled || reasoningEffort) {
      body.reasoning = {};
      if (thinkingEnabled) body.reasoning.summary = "auto";
      if (reasoningEffort) {
  ...
  parseSSELine(data, eventType) {
    ...
    if (type === "response.output_item.done") {
      ...
      return { type: "delta", responseItemDone: item };
    }
    ...
    if (
      ...
      if (Array.isArray(response.output))
        result.responseItems = response.output;
    ...
    if (
      ...
      const error = obj.error || (obj.response && obj.response.error) || {};
      return {
        type: "error",
        error: {
          message: error.message || `Responses stream failed (${type})`,
          code: error.code || "responses_stream_error",
        },
      };
    }
    ...
    return {
      ...
      usage: obj.usage
        ? {
            ...
            cacheWrite: obj.usage.input_tokens_details?.cache_write_tokens || 0,
          }
        : null,
  ...
  commitContinuation(conversationKey, outputItems) {
    return responsesState.commit(conversationKey, outputItems);
  },

  continuationStatus() {
    return responsesState.status();
  },
};
...
const GeminiAdapter = {
  ...
  buildRequest(opts) {
    ...
    const body = {
      contents: _convertMessagesToGeminiContents(messages),
    };
    ...
    if (tools && tools.length) {
      ...
      if (toolChoice) {
        ...
        if (name)
          body.toolConfig.functionCallingConfig.allowedFunctionNames = [name];
      }
    }
    ...
    if (thinkingEnabled || thinkingBudget) {
      ...
      if (thinkingBudget) body.thinkingConfig.thinkingBudget = thinkingBudget;
    }
    if (stream) body.__dao_stream = true;
    return body;
  },

  buildRequestOpts(provCfg) {
    const headers = {
      "Content-Type": "application/json",
      Accept: "text/event-stream",
    };
    applyAuthHeaders(
      headers,
      Object.assign({}, provCfg, { protocol: PROTOCOL.GEMINI }),
    );
    return { headers };
  },
  ...
  parseSSELine(data) {
    ...
    for (const part of parts) {
      ...
      if (part.functionCall) {
        toolCalls.push({
          id: part.functionCall.id || _geminiCallId(part.functionCall.name),
          type: "function",
          function: {
    ...
    if (candidate?.finishReason)
      result.finishReason = _geminiFinishReason(candidate.finishReason);
    if (obj.usageMetadata) {
      result.usage = {
        input: obj.usageMetadata.promptTokenCount || 0,
        output: obj.usageMetadata.candidatesTokenCount || 0,
        cached: obj.usageMetadata.cachedContentTokenCount || 0,
      };
    }
    return result;
  },
  ...
  parseUnaryResponse(body) {
    ...
    const parsed = this.parseSSELine(JSON.stringify(obj));
    if (!parsed || parsed.type === "skip") return null;
    return {
  ...
  getCompletionPath(provCfg, model, stream = true) {
    if (/generateContent/i.test(provCfg.completionPath || ""))
      return String(provCfg.completionPath).replace(
        /\{model\}/g,
...
function _geminiCallId(name) {
  return (
    "call_" +
    Buffer.from(String(name || "tool"))
      .toString("hex")
      .slice(0, 20)
  );
}

function _geminiFinishReason(reason) {
...
function _convertMessagesToGeminiContents(messages) {
  const contents = [];
  for (const message of messages || []) {
        ...
        parts: [
          {
            functionResponse: {
              ...
              response: _jsonObjectOrValue(message.content),
            },
          },
        ],
      });
    ...
    const parts = _openAIContentToGeminiParts(message.content);
    for (const toolCall of message.tool_calls || []) {
      ...
      parts.push({
        functionCall: {
          ...
          name: fn.name || "",
          args: _jsonObjectOrValue(fn.arguments),
        },
      });
    }
...
function _openAIContentToGeminiParts(content) {
...
function _jsonObjectOrValue(value) {
...
function initAdapters(opts = {}) {
  _log = opts.log || (() => {});
  _log(
    "[adapters] 初始化完成 · 支持 openai-chat + anthropic + openai-responses + gemini",
  );
}
...
function _convertToolToResponses(tool) {
  if (tool && tool.type && tool.type !== "function")
 ...
 */
function _convertResponsesContent(content) {
  ...
  for (const part of content) {
    ...
    if (part.type === "image_url" && part.image_url) {
      ...
      const converted = { type: "input_image", image_url: imageUrl };
      if (typeof image === "object" && image.detail)
        converted.detail = image.detail;
      parts.push(converted);
      continue;
    }
    // Preserve already-normalized provider blocks, but never forward the
    // Chat Completions image_url wrapper to a Responses endpoint.
...
function _convertMessagesToResponsesInput(messages) {
  const input = [];
  ...
  for (const m of messages || []) {
    ...
    if (m.role === "assistant") {
      ...
      // 文本内容
      if (m.content) item.content = _convertResponsesContent(m.content);
      // 工具调用
      if (Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
        item.content = _convertResponsesContent(m.content || "");
      }
      if (item.content) input.push(item);
      ...
      input.push({
        role: m.role || "user",
        content: _convertResponsesContent(m.content || ""),
      });
    }
  }
...
function _hasAnthropicHourCache(value) {
  if (!value || typeof value !== "object") return false;
  if (Array.isArray(value)) return value.some(_hasAnthropicHourCache);
  if (value.cache_control && value.cache_control.ttl === "1h") return true;
  return Object.values(value).some(_hasAnthropicHourCache);
}

/**
 * 组合 Anthropic Beta 头
 */
function _composeAnthropicBetaHeader(provCfg, body) {
  const tokens = new Set(["prompt-caching-2024-07-31"]);
  // ★ 合并用户在 extraHeaders 中已配置的 anthropic-beta token
  //   道义: 二十八章「知其白守其辱」· 知默认缓存之白 · 守用户自配之辱 · 不可覆盖
  //   bug: 旧版直接 return 固定串 → 覆盖了用户配的 context-1m 等 beta token → 上游 400
  const userBeta =
    provCfg.extraHeaders && provCfg.extraHeaders["anthropic-beta"];
  if (typeof userBeta === "string" && userBeta.trim()) {
    for (const t of userBeta.split(",")) {
      const trimmed = t.trim();
      if (trimmed) tokens.add(trimmed);
    }
  }
  if (provCfg.thinkingEnabled) tokens.add("interleaved-thinking-2025-05-14");
  if (_hasAnthropicHourCache(body)) tokens.add("extended-cache-ttl-2025-04-11");
