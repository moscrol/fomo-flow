...
class MockLLM {
  ...
  async _h(q, s) {
    ...
    const hasToolValidationError = (o.messages || []).some(
      (x) =>
        x.role === "tool" &&
        /\[TOOL_ARGUMENT_ERROR\]/.test(String(x.content || "")),
    );
    const sc = hasToolValidationError
      ? this.sc.get("after_tool_validation") || this.sc.get("default")
      : hasToolResult
  ...
  _m(msg, tools) {
    ...
    if (
      /malformed.*edit|edit.*missing.*arg/i.test(msg) &&
      sc.has("malformed_edit")
    )
      return sc.get("malformed_edit");
